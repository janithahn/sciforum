...
export const UpdatePostComment = (state = {
        status: 'idle',
        errMess: null,
        response: {}
    }, action) => {
    switch(action.type) {
        case ActionTypes.UPDATE_POST_COMMENT_SUCCESS:
            return {...state, status: 'succeeded', errMess: null, response: action.payload}

        case ActionTypes.UPDATE_POST_COMMENT_LOADING:
            return {...state, status: 'loading', errMess: null, response: state.response}
        
        case ActionTypes.UPDATE_POST_COMMENT_FAILED:
...
export const UpdateAnswerComment = (state = {
        status: 'idle',
        errMess: null,
        response: {}
    }, action) => {
    switch(action.type) {
        case ActionTypes.UPDATE_ANSWER_COMMENT_SUCCESS:
            return {...state, status: 'succeeded', errMess: null, response: action.payload}

        case ActionTypes.UPDATE_ANSWER_COMMENT_LOADING:
            return {...state, status: 'loading', errMess: null, response: state.response}
        
        case ActionTypes.UPDATE_ANSWER_COMMENT_FAILED:
            return {...state, status: 'failed', errMess: action.payload, response: state.response}
